...
export function Sun({ sunDir, distance = 4200 }: { sunDir: THREE.Vector3; distance?: number }) {
  ...
  useFrame(({ camera }) => {
    sprite.current.position.copy(camera.position).addScaledVector(sunDir, distance);
    mat.current.opacity = Math.max(0, (weather.params.sun - 0.25) / 0.75);
  });

  return (
    <sprite ref={sprite} scale={[560, 560, 1]} renderOrder={-1}>
